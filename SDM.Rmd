...
vegetation <- read.table(occurence,  header=TRUE,  sep=',')
vegetation <- vegetation[,-1]
presvals <- extract(predictors, vegetation)
set.seed(0)
...
set.seed(0)
group <- kfold(occurence, 5)
pres_train <- occurence[group != 1, ]
pres_test <- occurence[group == 1, ]
```
